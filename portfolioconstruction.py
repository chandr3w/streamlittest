...
# App Title
st.set_page_config(page_title="Atas VC Portfolio Simulator", page_icon="https://atas.vc/img/favicon.png")
...
fund_size = st.sidebar.slider('Fund Size ($MM)', 5, 500, 10, step=5)
...
check_sizes['Seed'] = st.sidebar.slider(f'Check Size Range Seed', 0.25, 10.0, (0.5, 0.9), step=0.1)
...
for i in range(stage_index, len(stages)-1):
    if i==0:
        prob_advancement[stages[i]+' to '+stages[i+1]] = st.sidebar.slider(f'{stages[i]} → {stages[i+1]}', 0, 100, 80, step=1)
...
for stage in valid_stages + ['Series C', 'IPO']:
    if stage == 'Pre-Seed':
        exit_valuations[stage] = st.sidebar.slider(f'Exit Valuation at {stage}', 2, 20, (3, 8), step=1)
...
for col, metric, val in zip(
    row1,
    ["Avg. Paid-in", "Avg. Distributed", "Avg. MOIC", "IRR (Median %)"],
    [
        np.mean(paid_in),
